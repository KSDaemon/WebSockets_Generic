...
WebSocketsServer::WebSocketsServer(const uint16_t &port, const String &origin,
...
*/
void WebSocketsServerCore::onEvent(WebSocketServerEvent cbEvent) { _cbEvent = cbEvent; }

/*
   Sets the custom http header validator function
   @param httpHeaderValidationFunc WebSocketServerHttpHeaderValFunc ///< pointer to the custom http
   header validation function
   @param mandatoryHttpHeaders[] const char* ///< the array of named http headers considered to be
   mandatory / must be present in order for websocket upgrade to succeed
...
/*
   ...
   @return true if ok
*/
bool WebSocketsServerCore::sendTXT(const uint8_t &num, uint8_t *payload, size_t length,
...
bool WebSocketsServerCore::sendTXT(const uint8_t &num, char *payload, size_t length,
                                   bool headerToPayload) {
...
/**
   ...
   @return true if ok
*/
bool WebSocketsServerCore::sendBIN(const uint8_t &num, uint8_t *payload, size_t length,
                                   bool headerToPayload) {
    if (num >= WEBSOCKETS_SERVER_CLIENT_MAX) {
        return false;
    }
...
WSclient_t *WebSocketsServerCore::newClient(WEBSOCKETS_NETWORK_CLASS *TCPclient) {
    ...
    (WEBSOCKETS_NETWORK_TYPE == NETWORK_ESP32) || (WEBSOCKETS_NETWORK_TYPE == NETWORK_RTL8720DN)
            ...
            client->tcp->onDisconnect(std::bind(
                [](WebSocketsServerCore *server, AsyncTCPbuffer *obj, WSclient_t *client) -> bool {
            ...
            client->tcp->readStringUntil(
                '\n', &(client->cHttpLine),
                std::bind(&WebSocketsServerCore::handleHeader, this, client, &(client->cHttpLine)));
...
/**
   ...
   @param length size_t
*/
void WebSocketsServerCore::messageReceived(WSclient_t *client, WSopcode_t opcode, uint8_t *payload,
...
void WebSocketsServerCore::handleClientData() {
    ...
    for (uint8_t i = 0; i < WEBSOCKETS_SERVER_CLIENT_MAX; i++) {
        ...
        // if ( clientIsConnected(client) && client->cHttpHeadersValid )
        {
            ...
            if (len > 0) {
                // KH New
                WS_LOGINFO4("[handleClientData] Client:", client->num,
                            ", tcp->available len:", len);
                ...
                switch (client->status) {
                    case WSC_HEADER: {
                        ...
                        WS_LOGINFO2(
                            client->num,
                            "[handleClientData] =================== Start =======================");

                        String headerLine = client->tcp->readStringUntil('\n');

                        // KH New
                        WS_LOGINFO4("[handleClientData] Status WSC_HEADER. Client:", client->num,
                    ...
                    case WSC_CONNECTED:
                        // KH New
                        WS_LOGINFO2(
                            "[handleClientData] Status WSC_CONNECTED. handleWebsocket. Client:",
                            client->num);
                    ...
                    default:
                        // KH New
                        WS_LOGINFO4("[handleClientData] default: clientDisconnect. Client:",
                                    client->num, "unknown client status", client->status);
...
void WebSocketsServerCore::handleHeader(WSclient_t *client, String &headerLine) {
    ...
    if (headerLine.length() > 0) {
        ...
        if (ok) {
            ...
            WS_LOGDEBUG4("[handleHeader] Client:", client->num,
                         ", handshake:", (char *)handshake.c_str());
            ...
            runCbEvent(client->num, WStype_CONNECTED, (uint8_t *)client->cUrl.c_str(),
                       client->cUrl.length());
...
*/
void WebSocketsServerCore::enableHeartbeat(const uint32_t &pingInterval,
