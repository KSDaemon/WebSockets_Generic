...
class WebSocketsClient : protected WebSockets {
  public:
...
#else
    typedef std::function<void(WStype_t type, uint8_t *payload, size_t length)>
    ...
    void begin(const char *host, const uint16_t &port, const char *url = "/",
               const char *protocol = "arduino");
    void begin(const String &host, const uint16_t &port, const String &url = "/",
               const String &protocol = "arduino");
    void begin(const IPAddress &host, const uint16_t &port, const char *url = "/",
               const char *protocol = "arduino");

    // KH
    void begin(const IPAddress &host, const uint16_t &port, const String &url = "/",
               const String &protocol = "arduino");

#if defined(HAS_SSL)
#ifdef SSL_AXTLS

    void beginSSL(const char *host, const uint16_t &port, const char *url = "/",
                  const char *fingerprint = "", const char *protocol = "arduino");

    void beginSSL(const IPAddress &host, const uint16_t &port, const String &url = "/",
                  const String &fingerprint = "", const String &protocol = "arduino");

    void beginSSL(const String &host, const uint16_t &port, const String &url = "/",
                  const String &fingerprint = "", const String &protocol = "arduino");
    ...
    void beginSslWithCA(const char *host, const uint16_t &port, const char *url = "/",
                        BearSSL::X509List *CA_cert = NULL, const char *protocol = "arduino");

    // New in v2.3.5
    void setSSLClientCertKey(BearSSL::X509List *clientCert         = NULL,
                             BearSSL::PrivateKey *clientPrivateKey = NULL);
    ...
    void beginSocketIO(const char *host, const uint16_t &port,
                       const char *url = "/socket.io/?EIO=4", const char *protocol = "arduino");

    void beginSocketIO(const String &host, const uint16_t &port,
                       const String &url = "/socket.io/?EIO=4", const String &protocol = "arduino");

    void beginSocketIO(const IPAddress &host, const uint16_t &port,
                       const String &url = "/socket.io/?EIO=4", const String &protocol = "arduino");

#if defined(HAS_SSL)
    void beginSocketIOSSL(const char *host, const uint16_t &port,
                          const char *url = "/socket.io/?EIO=4", const char *protocol = "arduino");

    void beginSocketIOSSL(const String &host, const uint16_t &port,
                          const String &url      = "/socket.io/?EIO=4",
                          const String &protocol = "arduino");

    void beginSocketIOSSL(const IPAddress &host, const uint16_t &port,
                          const String &url      = "/socket.io/?EIO=4",
                          const String &protocol = "arduino");

    void beginSocketIOSSLWithCA(const char *host, const uint16_t &port,
                                const char *url = "/socket.io/?EIO=4", const char *CA_cert = NULL,
                                const char *protocol = "arduino");

#if defined(SSL_BARESSL)
    void beginSocketIOSSLWithCA(const char *host, const uint16_t &port,
                                const char *url            = "/socket.io/?EIO=4",
                                BearSSL::X509List *CA_cert = NULL,
                                const char *protocol       = "arduino");
#endif // SSL_BARESSL
