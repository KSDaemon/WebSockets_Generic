...
typedef enum {
    ...
    eIOtype_CLOSE = '1', ///< Request the close of this transport but does not shutdown the
                         ///< connection itself.
    eIOtype_PING  = '2', ///< Sent by the client. Server should answer with a pong packet containing
                         ///< the same data
    eIOtype_PONG  = '3', ///< Sent by the server to respond to ping packets.
    eIOtype_MESSAGE = '4', ///< actual message, client and server should call their callbacks
                           ///< with the data
    eIOtype_UPGRADE = '5', ///< Before engine.io switches a transport, it tests, if server and
                           ///< client can communicate over this transport. If this test succeed,
                           ///< the client sends an upgrade packets which requests the server to
                           ///< flush its cache on the old transport and switch to the new
                           ///< transport.
    eIOtype_NOOP    = '6', ///< A noop packet. Used primarily to force a poll cycle when an incoming
...
class SocketIOclient : protected WebSocketsClient {
  public:
#ifdef __AVR__
    typedef void (*SocketIOclientEvent)(socketIOmessageType_t type, uint8_t *payload,
                                        size_t length);
#else
    typedef std::function<void(socketIOmessageType_t type, uint8_t *payload, size_t length)>
        SocketIOclientEvent;
#endif
    ...
    void beginSSL(const String &host, const uint16_t &port, const String &url = "/socket.io/?EIO=4",
                  const String &protocol = "arduino");
#ifndef SSL_AXTLS
    void beginSSLWithCA(const char *host, const uint16_t &port,
                        const char *url = "/socket.io/?EIO=4", const char *CA_cert = NULL,
                        const char *protocol = "arduino");

    void beginSSLWithCA(const char *host, const uint16_t &port,
                        const char *url = "/socket.io/?EIO=4", BearSSL::X509List *CA_cert = NULL,
                        const char *protocol = "arduino");
    ...
    bool send(socketIOmessageType_t type, uint8_t *payload, size_t length = 0,
              bool headerToPayload = false);
    bool send(socketIOmessageType_t type, const uint8_t *payload, size_t length = 0);
    bool send(socketIOmessageType_t type, char *payload, size_t length = 0,
              bool headerToPayload = false);
    ...
    inline void setReconnectInterval(const unsigned long &time) { _reconnectInterval = time; }
